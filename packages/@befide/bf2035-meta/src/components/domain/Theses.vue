<template>
	<div class="card">
		...
		>
			...
			<Column header="Typ" :sortable="true" field="type.data.label.fullName.en"> </Column>
...
onMounted(async () => {
	data.value = await fetch('/api/theses.json').then((response) => response.json());

	universities.value = Array.from(new Set(data.value.map((d) => d.data.university)));
