...
const StarlightPageFrontmatterSchema = async (context: SchemaContext) => {
  ...
  const schema =
    typeof userDocsSchema === 'function'
      ? userDocsSchema(context)
      : userDocsSchema;
  ...
  return schema.transform((frontmatter) => {
    ...
    const pageEditUrl =
      editUrl === undefined || editUrl === true ? false : editUrl;
...
export type StarlightPageProps = Prettify<
  ...
  Partial<
    Omit<
      ...
      'entry' | 'entryMeta' | 'id' | 'locale' | 'slug'
    >
  > &
...
export async function generateStarlightPageRouteData({
  ...
  const id = project.legacyCollections
    ? `${stripLeadingAndTrailingSlashes(slug)}.md`
    : slug;
  const localeData = slugToLocaleData(slug);
  const sidebar = props.sidebar
    ? getSidebarFromConfig(
        validateSidebarProp(props.sidebar),
        url.pathname,
        localeData.locale,
      )
  ...
  const editUrl = pageFrontmatter.editUrl
    ? new URL(pageFrontmatter.editUrl)
    : undefined;
  const lastUpdated =
    pageFrontmatter.lastUpdated instanceof Date
      ? pageFrontmatter.lastUpdated
      : undefined;
...
async function getStarlightPageFrontmatter(
  frontmatter: StarlightPageFrontmatter,
) {
...
> {
  const userCollections = (await import('virtual:starlight/collection-config'))
    .collections;
