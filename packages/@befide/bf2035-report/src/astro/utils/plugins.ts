import type {
  AstroIntegration,
  HookParameters as AstroHookParameters,
} from 'astro';
...
export async function runPlugins(
  ...
  const routeMiddlewareConfigs: Array<
    z.output<typeof routeMiddlewareConfigSchema>
  > = [];
    ...
    if (i18nSetup) {
      await i18nSetup({
        injectTranslations(translations) {
          ...
          for (const [locale, localeTranslations] of Object.entries(
            translations,
          )) {
  ...
  for (const {
    ...
    await setup({
      config: pluginsUserConfig
        ? { ...userConfig, plugins: pluginsUserConfig }
        : userConfig,
...
function applyPluginMiddleware(
  ...
  starlightConfig.routeMiddleware.push(
    ...middlewareBuckets.default,
    ...middlewareBuckets.post,
  );
}
    ...
    z.tuple([
      z.object({
        ...
        config: z.any() as z.Schema<
          ...
          StarlightUserConfig & {
            plugins?: z.input<typeof baseStarlightPluginSchema>[];
          }
        >,
        /**
        ...
        addRouteMiddleware: z.function(
          z.tuple([routeMiddlewareConfigSchema]),
          z.void(),
        ),
        /**
        ...
        useTranslations: z.any() as z.Schema<
          ReturnType<typeof createTranslationSystemFromFs>
        >,
        /**
...
export type PluginTranslations = Record<
  string,
  UserI18nSchema & Record<string, string>
>;
