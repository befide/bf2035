...
function getRoutePathRelativeToCollectionRoot(
  route: Route,
  locale: string | undefined,
) {
  localizedId(
...
function treeify(
  ...
  routes
    ...
    .map(
      (doc) =>
        [getRoutePathRelativeToCollectionRoot(doc, locale), doc] as const,
    )
...
export function getSidebarFromConfig(
  ...
  const intermediateSidebar = getIntermediateSidebarFromConfig(
    sidebarConfig,
    pathname,
    locale,
  );
