...
import { z } from 'zod';
import { NullableLocalizedString, readInputFile, ReviewSchema } from './content.config.common';

export const CourseSchema = z.object({
	id: z.string(),
	isInstanceOfTeachingEvent: reference('taxonomyItems').optional(),
	addressesProgrammeOfStudyLevels: z.array(reference('taxonomyItems')).optional().nullable(),
	...
	loader: () => {
		...
		data.forEach((d: any) => {
			if (d.addressesProgrammeOfStudyLevels) {
				d.addressesProgrammeOfStudyLevels = d.addressesProgrammeOfStudyLevels.split(/\s?,\s?/);
			}
		});
		return data;
	},
