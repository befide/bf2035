						...
						>
							...
							<div class="pt-6">
								<div class="flex flex-row justify-between items-start gap-2">
									<div>
										<span class="font-medium text-surface-500 dark:text-surface-400 text-xl">{{
											item.data.labelShortFormDe
										}}</span>
...
onMounted(async () => {
	taxonomy.value = await fetch('/api/taxonomy.json').then((response) => response.json());
});
// const columns = ref([
