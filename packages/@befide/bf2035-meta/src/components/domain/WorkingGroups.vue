		...
		>
			<ColumnGroup type="header">
				<Row>
					...
					<Column header="Group name" :rowspan="4" :sortable="true" fieldName="label.fullName.en" />
...
const aggregationModes = ['position/discipline/gender', 'gender', 'discipline/gender'];
...
onMounted(async () => {
	data.value = await fetch('/api/working-groups.json').then((response) => response.json());
	const partOfIds = Array.from(new Set(data.value.map((d) => d.data.partOf.id)));
