...
const UserConfigSchema = z.object({
  ...
  disable404Route: z
    .boolean()
    .default(false)
    .describe("Disable Starlight's default 404 page."),
  ...
  routeMiddleware: z
    ...
    .superRefine((middlewares, ctx) => {
      ...
      const invalidPaths = middlewares.filter((middleware) =>
        invalidPathRegex.test(middleware),
      );
  ...
  .transform(({ title, locales, defaultLocale, ...config }, ctx) => {
    ...
    if (
      ...
      if (!defaultLocaleConfig) {
        const availableLocales = configuredLocales
          .map((l) => `"${l}"`)
          .join(', ');
        ctx.addIssue({
          code: 'custom',
      ...
      // Transform the title
      const TitleSchema = TitleTransformConfigSchema(
        defaultLocaleConfig.lang as string,
      );
